...
try:
    ...
    from panos.network import (
        ...
        BgpPolicyAdvertiseFilter,
        BgpPolicyAggregationAddress,
        BgpPolicyConditionalAdvertisement,
        BgpPolicyNonExistFilter,
        BgpPolicySuppressFilter,
        VirtualRouter,
    )
except ImportError:
    try:
...
def setup_args():
    return dict(
        ...
        policy_type=dict(
            type="str",
            required=True,
            choices=["conditional-advertisement", "aggregate"],
        ),
...
def main():
    ...
    if module.params["filter_type"] == "non-exist":
        ...
        module.fail_json(
            msg="Unknown filter_type: {0}".format(module.params["filter_type"])
        )
